...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    try:
        ...
        torch.onnx.export(model, im, f, verbose=True, opset_version=opset,
...
def export_engine(model, im, file, train, half, simplify,max_batch_size,precision='fp16',workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    try:
        check_requirements(('tensorrt',))
        import tensorrt as trt

        opset = (12, 13)[trt.__version__[0] == '8']  # test on TensorRT 7.x and 8.x
        export_onnx(model, im, file, opset, train, True, simplify=True)
        ...
        # inspector = trt.EngineInspector()
        # inspector.execution_context=builder.create_execution_context()
        #add dynamic batch
        ...
        # #     builder.int8_calibrator = Calibrator(cache_file=cache_file)
        # elif precision == 'fp32':
        #     builder.platform_has_fast_fp32=True
        # else:
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    if 'engine' in include:
        export_engine(model, im, file, train, half, simplify, batch_size,workspace, verbose)
